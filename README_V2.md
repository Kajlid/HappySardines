...
**[Try the live demo](https://huggingface.co/spaces/AxelHolst/HappySardines)** | **[Monitoring Dashboard](https://huggingface.co/spaces/AxelHolst/HappySardines/Monitoring)**
...
flowchart TB
    subgraph Data Sources
        KODA[KODA API<br/>GTFS-RT positions]
        WEATHER[Open-Meteo<br/>Weather forecasts]
        HOLIDAYS[Svenska Dagar<br/>Swedish holidays]
    end

    subgraph GitHub Actions
        FEAT[Feature Pipeline<br/>Daily 10:00 UTC]
        HEAT[Heatmap Pipeline<br/>Daily 05:00 UTC]
        TRAIN[Training Pipeline<br/>Manual trigger]
        INF[Inference Pipeline<br/>After features]
    end
    ...
    subgraph UI
        APP[Streamlit App<br/>HuggingFace Spaces]
    end
...
We use an XGBoost classifier (`XGBClassifier` from [xgboost](https://xgboost.readthedocs.io/)) trained on approximately 5 million trip-window samples.

### Why XGBoost

We chose XGBoost over deep learning approaches for several reasons:

1. **Speed**: Trains in ~5 minutes on our dataset (vs hours for neural networks)
2. **Interpretability**: Feature importance is directly available
3. **Handles mixed types**: Works well with both categorical and numeric features
4. **Proven track record**: Mature library, widely used in production systems
5. **Works with limited data**: Less data-hungry than deep learning
...
XGBOOST_PARAMS = {
    ...
    "objective": "multi:softprob", # Probabilities for all classes
    "num_class": 7,              # All GTFS-RT occupancy classes
}
```
...
- **Probability output**: Using `multi:softprob` gives us confidence scores, not just hard predictions
...
**Per-class recall:**
- Class 0 (Empty): 66.8%
- Class 1 (Many seats): 57.3%
- Class 2 (Few seats): **56.4%**
- Class 3 (Standing): 7.4%

The key improvement in v4 was achieving 56% recall on class 2 (previously 0%), meaning the model now actually predicts when buses will have few seats available. Class 3 recall remains low due to extreme rarity, but is no longer zero.
...
- **Interactive map**: Click anywhere in Östergötland to get a crowding prediction for that location
- **Heat map overlay**: Toggle to see predicted crowding levels across the entire region as colored grid cells
- **Time selection**: Choose any hour (5-23) for today or tomorrow
- **Weather display**: Shows current temperature and weather conditions (rain/snow warnings)
- **Holiday awareness**: Indicates if the selected day is a Swedish holiday or work-free day
- **Bus information**: Displays nearest stop and route details when available

**Occupancy level display:**
| Level | Color | Message |
|-------|-------|---------|
| Empty | Green | "Plenty of room - pick any seat!" |
| Many seats | Green | "Lots of seats to choose from." |
| Few seats | Yellow | "Some seats left - you might need to look around." |
| Standing | Orange | "Expect to stand - pack your patience!" |
| Crushed/Full | Red | "Very crowded - consider waiting for the next one." |

### Monitoring Dashboard

A separate [Monitoring page](https://huggingface.co/spaces/AxelHolst/HappySardines/Monitoring) shows:

- **Accuracy over time**: Line chart tracking daily model performance
- **Per-class recall**: Progress bars showing how well each occupancy level is predicted
- **Hourly comparison**: Actual vs predicted occupancy by hour of day
- **Alert threshold**: Warning when accuracy drops below 65%
- **Model version filter**: Compare performance across different model versions
